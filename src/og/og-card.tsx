...
type OgCardProps = {
  data: PortfolioData;
  /** Optimized portrait as a base64 data-URI (read at module scope in og-image.tsx). */
  portraitSrc: string;
};

/**
 * The Hero-mirroring OG image card: availability badge, mono role line,
 * gradient last name, tagline, CTAs, social chips, portrait with the floating
 * stat badge, and the skills marquee strip. Rendered with inline styles only
 * so it works inside `ImageResponse` (Satori requires explicit `display: flex`).
 */
export function OgCard({ data, portraitSrc }: OgCardProps) {
